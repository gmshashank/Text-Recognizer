...
ESSENTIALS_FILENAME = Path(__file__).parents[0].resolve() / "emnist_lines_eesentials.json"
...
def create_dataset_of_images(N, sampples_by_char, sentence_generator, min_overlap, max_overlap, dims):
    images = torch.zeros((N, dims[1], dims[2]))
    labels = []
    for n in range(N):
        label = sentence_generator.generate()
        images[n] = convert_strings_to_labels(label, sampples_by_char, min_overlap, max_overlap, dims[-1])
        labels.append(label)
...
def select_letter_samples_for_string(string, samples_by_char):
    ...
    for char in string:
        if char in sample_image_by_char:
            continue
        samples = sample_image_by_char[char]
        sample = samples[np.random.choice(len(samples))] if samples else zero_image
        sample_image_by_char[char] = sample.resahpe(28, 28)
...
class EMNISTLines(BaseDataModule):
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument(
            ...
            type=float,
            default=MIN_OVERLAP,
            help="Min overlao between characters in a line, between 0 and 1.",
        )
        parser.add_argument(
            "--max_overlap",
            type=float,
    ...
    def data_filename(self):
        return (
            DATA_DIRNAME
            / f"ml_{self.max_length}_o{self.min_overlap:f}_{self.max_overlap:f}_ntr{self.num_train}_ntv{self.num_val}_nte{self.with_start_end_tokens}.h5"
        )

    def _generate_data(self, split: str) -> None:
        print("EMNISTLineaDataset generating data for {split}.")

        from text_recognizer.data.sentence_generator import SentenceGenerator

        sentence_generator = SentenceGenerator(self.max_length - 2)
        emnist = self.emnist
        emnist.prepare_data()
        emnist.step()
    ...
    def setup(self, stage: str = None) -> None:
        ...
        if stage == "test" or stage is None:
            ...
            self.data_etst = BaseDataset(x_test, y_test, transform=self.transform)
